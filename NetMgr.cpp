...
NetMgr* NetMgr::s_pInstance = new NetMgr();
...
bool NetMgr::StartNetWork( uint16 port, const char* addr )
{
   ...
   //    }
    ...
    MyAceSvcHandler::MyAcceptor* acc = new MyAceSvcHandler::MyAcceptor( _addr, m_Reactor );
    m_pAcceptor = acc;
...
void NetMgr::StopNetWorld()
{
    if( m_pAcceptor )
    {
        MyAceSvcHandler::MyAcceptor* acc = dynamic_cast<  MyAceSvcHandler::MyAcceptor*> (m_pAcceptor);
        acc->close();

    }
    if ( m_pConnector )
    {
        MyAceSvcHandler::MyConnector* con = dynamic_cast< MyAceSvcHandler::MyConnector* >(m_pConnector);
        con->close();
    }
//    for ( int i = 0; i < 2; ++i )
//    {
